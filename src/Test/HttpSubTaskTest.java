...
public class HttpSubTaskTest {
    ...
    public void testAddSubTask() throws IOException, InterruptedException, TimeOverlapException {
        ...
        assertNotNull("Задачи не возвращаются", subTasks);
        assertEquals(manager.getSubTasks().size(), subTasks.size(), "Некорректное количество задач");
        assertEquals(manager.getSubTasks().get(6).getName(), subTasks.get(6).getName(), "Некорректное имя задачи");
