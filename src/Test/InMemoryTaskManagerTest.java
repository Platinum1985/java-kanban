...
class InMemoryTaskManagerTest {
    ...
    public void islookingATaskById() {
        ...
        manager.addSubTask(subTask, 1); // Пример связи подзадачи с эпиком
        // Получаем задачи по ID
        String taskById = manager.getTaskById(0);
        String epicById = manager.getEpicById(1);
        String subTaskById = manager.getSubTaskById(2);
    ...
    void testTaskImmutabilityAfterAdding() {
        ...
        // Получаем задачу из менеджера
        Epic addedEpic = ((InMemoryTaskManager) manager).getEpics().get(1);
        ...
        // Получаем задачу из менеджера
        SubTask addedSubTask = ((InMemoryTaskManager) manager).getSubTasks().get(2);
